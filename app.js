...
// ==========================================
// 🖱️ ระบบลากปุ่มเมนูลอย (Draggable Floating Dock)
// ==========================================
document.addEventListener('DOMContentLoaded', () => {
    const dock = document.getElementById('draggable-dock');
    ...
    // โหลดตำแหน่งเดิมที่เคยเซฟไว้
    const savedPos = JSON.parse(localStorage.getItem('floatingDockPos'));
    if (savedPos) {
        xOffset = savedPos.x; yOffset = savedPos.y;
        setTranslate(xOffset, yOffset, dock);
    }
    ...
    // รองรับจอสัมผัส (มือถือ/แท็บเล็ต)
    ...
    function dragEnd(e) {
        ...
        isDragging = false;
        // เซฟพิกัดเมื่อปล่อยเมาส์
        localStorage.setItem('floatingDockPos', JSON.stringify({x: xOffset, y: yOffset}));
    }
